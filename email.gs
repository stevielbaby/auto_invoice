...
function createInvoiceEmailDraft() {
  ...
  var clientSheet = spreadsheet.getSheetByName("Clients"); // Assume client details are in a sheet named "Client"
  ...
  var clientData = clientSheet.getRange("A:B").getValues();  // Assume column A contains client names and column B contains emails
  var email = "";
  for (var i = 0; i < clientData.length; i++) {
    if (clientData[i][0] === clientName) {
      email = clientData[i][1];
      break;
    }
  }
